...
$columnsResult = $conn->query("SHOW COLUMNS FROM `cselaptops`");
$columns = [];
...
<body>
                        ...
                        <div class="modal-body">
                            <form action="../includes/add_laptop.php" method="post">
                                <?php foreach ($columns as $column): ?>
                                    ...
                                    <?php endif; ?>
                                <?php endforeach; ?>
                                <input type="hidden" name="dept_id" value="<?php echo $dept_id; ?>"> <!-- Fixed department ID -->
    ...
    <div class="container mt-5">
        <h3 class="mt-4 mb-3 text-center text-success">CSE laptops Inventory</h3>
                    ...
                    <?php
                        ...
                        if ($result->num_rows > 0) {
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo "<td>
                                        <button onclick='updatelaptops(" . $row["id"] . ", \"" . implode('","', array_map(function($c) use ($row) { return $row[$c]; }, $columns)) . "\")' class='btn btn-primary'>Update</button>
                                        <br><br>
                                        <a href='../includes/delete_laptop.php?id=" . $row["id"] . "' class='btn btn-danger'>Delete</a>
                                        <br><br>
    ...
    <script>
        function updatelaptops(id, ...values) {
            $('#updateId').val(id);
