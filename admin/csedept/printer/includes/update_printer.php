...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_fetch = "SELECT * FROM cseprinters WHERE id = ?";
    ...
    if ($row) {
        ...
        $sql_update = "UPDATE cseprinters SET " . implode(', ', $updates) . " WHERE id = ?";
        ...
        if ($stmt_update->execute()) {
            ...
            $url = "http://yingathungkikon.000.pe/user/printer/display_printer.php?pr_number=" . urlencode($pr_number) . "&dept_id=3";
            // Generate the QR code with the URL
            $qrCodeImagePath = $qrCodeDir . 'qr_image_' . $pr_number . '.png';
            QRcode::png($url, $qrCodeImagePath);
            
            // Update the QR image path
            $sql_qr_update = "UPDATE cseprinters SET qr_img = ? WHERE id = ?";
