...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username)) {
        ...
        if ($stmt->execute()) {
            ...
            $sql_computers = "SELECT computers FROM departments WHERE dept_id = ?";
            $stmt_computers = $conn->prepare($sql_computers);
            $stmt_computers->bind_param("s", $dept_id);
            $stmt_computers->execute();
            $stmt_computers->bind_result($computers);
            $stmt_computers->fetch();
            $stmt_computers->close();
            ...
            $update_sql = "UPDATE {$computers} SET problem = ? WHERE pc_number = ?";
        ...
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <?php endif; ?>
                    <form method="post">
                        <!-- Add hidden input fields to include the PC number and dept_id -->
