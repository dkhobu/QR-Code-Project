...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT * FROM laptopreports WHERE id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_query = "UPDATE laptopreports SET lap_number = ?, dept_id = ?, report_message = ?, status = ? WHERE id = ?";
    $update_stmt = $conn->prepare($update_query);
    if ($update_stmt === false) {
        die('Prepare failed: ' . htmlspecialchars($conn->error));
    }
    $update_stmt->bind_param("ssssi", $lap_number, $device_type, $report_message, $status, $report_id);
...
<div class="container mt-5">
    ...
    <form method="POST">
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="lap_number" name="lap_number" value="<?php echo htmlspecialchars($row['lap_number']); ?>" required>
        </div>
        <div class="form-group">
